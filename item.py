...
class Item(pygame.sprite.Sprite):
    """
    This class creates interactable items
    that the player can hold and store in their hotbar.
    """
    ...
    def __init__(self, image, position, size):
        ...
        Inputs:
            ...
            position - a tuple representing the
            x and y coordinates of the sprite in blocks (32 pixels = 1 block)
        ...
        self.pos = [
            blocks_to_pixels.blocks_to_pixels(position[0]),
        ...
        self.image_reversed = pygame.transform.flip(
            self.image_normal, True, False
        )

    def pick_up(self, hotbar, player):
        """
        This method is run if the player picks
        the item up. It is then stored in the
        players selected hotbar slot
        Inputs:
        ...
        if self.mask.overlap(player.mask, offset):
            ...
            if accepted:
                # hide the world item and clear
                # its mask so it no longer collides or draws
                self.visible = False
                # clear mask so overlap won't trigger again;
                # recreate when returning to world
                self.mask.clear()

    def holding_item(self, player):
        """
        This determines if the item is selected and tells
        the view where to put the item on the player
        so it looks like steve is holding it.
        Inputs:
            player - instance of the player class
        """
        if player.image == player.imageright:
            # setting hold coordinates if steve is facing right
            self.hold_coords = self.hold_coords = [
                player.pos.x + 2,
                player.pos.y + 6,
            ]
