...
"""
import pygame
from pygame.locals import (K_LEFT,K_RIGHT,K_1,
K_2, K_3, K_4, K_5, K_6, K_7, K_8, K_9,K_SPACE)
...
class Controller(pygame.sprite.Sprite):
    ...
    def __init__(self, size):
        ...
        steve_right = pygame.image.load(
            "sprites\\player sprites\\Steve_Right.png"
        ).convert_alpha()
        self.imageright = pygame.transform.scale(steve_right, (8, 31))
        self.image = self.imageright
        # loading steves left side image
        steve_left = pygame.image.load(
            "sprites\\player sprites\\Steve_Left.png"
        ).convert_alpha()
        self.imageleft = pygame.transform.scale(steve_left, (8, 31))
        # creating steves mask which determines
        # whether he is overlapping other sprites
    ...
    def change_image(self):
        """
        Changes the image of steve from 
        left to right view when moving left or right.
        """
    ...
    def move(self, hotbar):
        ...
        # and right when keys are pressed
        if pressed_keys[K_LEFT]:
            self.acc.x = -ACC
        if pressed_keys[K_RIGHT]:
            self.acc.x = ACC
        # checks if they press one of the numbers selected
        # and telling the HOTBAR instance
        # which one has been selected
        for key, i in zip(number_keys, range(1, 10)):
            if pressed_keys[key]:
                hotbar.change_selected(i)
        # setting acceleration velocity
        #  and position using current and set values
        self.acc.x += self.vel.x * FRIC
        self.vel += self.acc
        self.pos += self.vel + 0.5 * self.acc
        # setting steves mask to be at
        # the same position as the image of steve
        self.mask.set_at((0, 0), 1)
        self.mask.set_at(
            (self.surf.get_width() - 1,
            self.surf.get_height() - 1), 1
        )

    def update(self, platforms, interactables, solid_mask):
        """
        This is the update method. It makes it so the 
        player doesnt fall through the floor 
        ...
        for temp_platform in platforms:
            if (
                self.mask.overlap(
                    temp_platform.mask,
                    [
                        temp_platform.pos[0] - self.pos.x,
                        temp_platform.pos[1] - self.pos.y - 1,
                    ],
                )
