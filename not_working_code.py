...
class Player(pygame.sprite.Sprite):
    ...
    def update(self):
        #checks if its touching a platform
        hits = pygame.sprite.spritecollide(self , platforms, False)
        interactable_hits = pygame.sprite.spritecollide(self, interactables, False)
...
class platform(pygame.sprite.Sprite):
    def __init__(self,image,position,size):
        ...
        img = pygame.image.load(image).convert_alpha()
        self.image = pygame.transform.scale(img, (blocks_to_pixels.blocks_to_pixels(size[0]),blocks_to_pixels.blocks_to_pixels(size[1])))
        self.rect = pygame.mask.from_threshold(self.image, 'red')
        self.visible = True
...
CAVE_ENTRANCE = platform("platform_cave_entrance.png",[25,16],[10,13])
P1 = Player((1,2))
